...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  9 15:38:10 2017
...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb  7 17:15:27 2017
...
start_time='2014-11-06 22:00:00'
end_time='2014-11-06 22:45:00'
...
while start<len(data):
     ...
     for l in range(2,9):
         ...
         cal_avg = metrics.calinski_harabaz_score(data1[['Ddeq','V','Aratio','ASPratio']], labels=cluster_labels)
         cal_value_GM.append(cal_avg)
         #print("For n_clusters =", l,
         #     "The average silhouette_score is :", silhouette_value_GM)
         if (cal_avg > cal_GM_max):
             cal_GM_max=cal_avg
             n_cluster_GM=l
         ...
         #it's necessary a treshold to decide if are better 2 or 3 clusters
         #treshold means that the silhouette coefficient is very similar
         #for 2 and 3 the algorithm try to force 2 clusters also if 3 is better
         if(l==3):       
             if(cal_GM_max-cal_avg<0.1 and cal_avg>0.3
                and cal_GM_max<0.4):
                 n_cluster_GM=l
     ...
     plt.subplot(3, 1, 1)
     plt.plot(range(1,9), cal_value_GM,'ro-',label='K-Means')
     ...
     if(needles==0):
        n_needles.append(len(GM_cluster0))
     if(needles==1):
        n_needles.append(len(GM_cluster1))
     if(needles==2):
        n_needles.append(len(GM_cluster2))
     elif(needles==50):
        n_needles.append(0)
        
     if(needlesaggr==0):
        n_needlesaggr.append(len(GM_cluster0))
     if(needlesaggr==1):
        n_needlesaggr.append(len(GM_cluster1))
     if(needlesaggr==2):
        n_needlesaggr.append(len(GM_cluster2))
     elif(needlesaggr==50):
        n_needlesaggr.append(0)
        
     if(graupeln==0):
        n_graupeln.append(len(GM_cluster0))
     if(graupeln==1):
        n_graupeln.append(len(GM_cluster1))
     if(graupeln==2):
        n_graupeln.append(len(GM_cluster2))
     elif(graupeln==50):
        n_graupeln.append(0)
     ...
     plt.ylabel(y_variable)
     if(needles==0):
         plt.title('GM Needles %s' %len(GM_cluster0))
         yes_needles.append(100)
     if(needlesaggr==0):
         plt.title('GM Needles Aggregate %s' %len(GM_cluster0))
         yes_needlesaggr.append(100)
     if(graupeln==0):
         plt.title('GM Graupeln %s' %len(GM_cluster0))
     ...
     plt.ylabel(y_variable)
     if(needles==1):
         plt.title('GM Needles %s' %len(GM_cluster1))
         yes_needles.append(100)
     if(needlesaggr==1):
         plt.title('GM Needles Aggregate %s' %len(GM_cluster1))
         yes_needlesaggr.append(100)
     if(graupeln==1):
         plt.title('GM Graupeln %s' %len(GM_cluster1))
         yes_graupeln.append(100)
     if(needles==50 and needlesaggr==50 and graupeln==50):
         plt.text(1,0.5,'Clustering Error')
     ...
     plt.ylabel(y_variable)
     if(needles==2):
         plt.title('GM Needles %s' %len(GM_cluster2))
         yes_needles.append(100)
     if(needlesaggr==2):
         plt.title('GM Needles Aggregate %s' %len(GM_cluster2))
         yes_needlesaggr.append(100)
     if(graupeln==2):
         plt.title('GM Graupeln %s' %len(GM_cluster2))
         yes_graupeln.append(100)
     if(needles==50 and needlesaggr==50 and graupeln==50):
         plt.text(1,0.5,'Clustering Error')
...
'''
#Plot number of particle of each cluster
plt.subplot(4, 1, 2, sharex=ax1)
plt.plot(time, n_needles, 'ro-')
plt.ylim(0,6000) 
plt.xlim(datetime.datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S')-datetime.timedelta(minutes=2),
         datetime.datetime.strptime(end_time, '%Y-%m-%d %H:%M:%S')+datetime.timedelta(minutes=2))    
plt.xlabel('Time')
...
plt.plot(time, n_needlesaggr, 'go-')
plt.ylim(0,6000) 
plt.xlim(datetime.datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S')-datetime.timedelta(minutes=2),
         datetime.datetime.strptime(end_time, '%Y-%m-%d %H:%M:%S')+datetime.timedelta(minutes=2))    
plt.xlabel('Time')
plt.ylabel('Aggregates')
fig4.autofmt_xdate()
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d %H:%M:%S'))

plt.subplot(4, 1, 4, sharex=ax1)
plt.plot(time, n_graupeln, 'bo-')
plt.ylim(0,6000) 
plt.xlim(datetime.datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S')-datetime.timedelta(minutes=2),
         datetime.datetime.strptime(end_time, '%Y-%m-%d %H:%M:%S')+datetime.timedelta(minutes=2))    
plt.xlabel('Time')
