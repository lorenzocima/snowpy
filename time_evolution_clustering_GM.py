...
while start<len(data):
     ...
     for l in range(2,4):
         ...
         if(l==3):       
             if(silhouette_GM_max-silhouette_avg<0.05 and silhouette_avg>0.3
                and silhouette_GM_max<0.4):
                 n_cluster_GM=l
             ...
             if(needlesaggr==2):
                 if(max(GM_cluster2['Ddeq'])-min(GM_cluster2['Ddeq'])>2.5):
                     if(max(Aratio_centroid)>0.65):
                         ...
                         graupeln=50
     '''
     if(n_cluster_GM==2 and needles!=50 and min(Ddeq_centroid)>1.1):
         needles=100
         n_cluster_GM=1
     ...
     max_silhouette.append(silhouette_GM_max)
...
figx=plt.figure()
ax1=plt.subplot(4, 1, 1)
plt.plot(time,max_silhouette,'k-')
plt.ylim(0.2,0.6) 
plt.xlim(datetime.datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S')-datetime.timedelta(minutes=2),
         datetime.datetime.strptime(end_time, '%Y-%m-%d %H:%M:%S')+datetime.timedelta(minutes=2))    
plt.xlabel('Time')
