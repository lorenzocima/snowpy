...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar  1 15:32:50 2017
...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 17 12:13:55 2017
...
##Necessary put video file from minute x1 or x6 to have the average every 5 minutes
##to compare directly with psd file. Also if there is a passage of midnight between
##different day the algorithm works well (23:56+23:57+23:58 -> 00:01+00:02+...)
...
filename='/home/cima/Documents/Helsinki Work/IC-PCA/Particle/classes/classes.csv'
...
#Link with directory 
particle_folder='/home/cima/Documents/Helsinki Work/IC-PCA/Particle'
...
#Move all the particle in the correct folder and remove the particle of bad crop
#because are already in a particular folder (bad_crop) thanks to matlab algorithm   
...
all_particle='yes' ### 'yes' for all the particle 
                   ### 'no' only particle of interest

percentage='no'### 'yes for the percentage
                ### 'no' for the number of particle  
...
for k in range(1, len(data)):
    ...
    if (current_time!=previous_time):
        ...
        if(stop_total=='end_day'):
            total.append(stop_total)
            stop_total='day'
        period=k
...
#Sum the number of particle to have the correct number for each 5 minutes step 

stop_kk=len(total)

for kk in range(0, stop_kk):
    step=5
    if(kk>=len(n_needles)):
        break
    for jj in range(0,5):
        if(kk+jj<len(total) and total[kk+jj]=='end_day'):
            step=jj
            del total[kk+jj]
    
    
    n_needles[kk]=sum(n_needles[kk:kk+step])
    n_needles_aggr[kk]=sum(n_needles_aggr[kk:kk+step])
    n_graupeln[kk]=sum(n_graupeln[kk:kk+step])
    n_small[kk]=sum(n_small[kk:kk+step])
    n_unclass[kk]=sum(n_unclass[kk:kk+step])
    total[kk]=sum(total[kk:kk+step])
    del n_needles[kk+1:kk+step]
    del n_needles_aggr[kk+1:kk+step]
    del n_graupeln[kk+1:kk+step]
    del n_small[kk+1:kk+step]
    del n_unclass[kk+1:kk+step]
    del total[kk+1:kk+step]
